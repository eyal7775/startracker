...
##        eq(conf.GetExtensionKeys('ZoomHeight'), {'<<zoom-height>>': key})
    ...
    def test_get_core_keys(self):
        ...
        eq(conf.GetCoreKeys()['<<history_data-next>>'], ['<Alt-n>'])
        ...
        eq(conf.GetCoreKeys('IDLE Classic Unix')['<<history_data-next>>'],
           ['<Alt-Key-n>', '<Meta-Key-n>'])
        eq(conf.GetCoreKeys('IDLE Modern Unix')['<<history_data-next>>'],
            ['<Alt-Key-n>', '<Meta-Key-n>'])

...
@unittest.skipUnless(hasattr(readline, "clear_history"),
                     "The history_data update test cannot be run because the "
                     "clear_history method is not available.")
class TestHistoryManipulation (unittest.TestCase):
    """
    These tests were added to check that the libedit emulation on OSX and the
    "real" readline have the same interface for history_data manipulation. That's
...
print("history_data", ascii(readline.get_history_item(1)))
"""
        ...
        # end, so don't expect it in the output.
        self.assertIn(b"history_data " + expected, output)

    # We have 2 reasons to skip this test:
    # - readline: history_data size was added in 6.0
    #   See https://cnswww.cns.cwru.edu/php/chet/readline/CHANGES
    # - editline: history_data size is broken on OS X 10.11.6.
    #   Newer versions were not tested yet.
    @unittest.skipIf(readline._READLINE_VERSION < 0x600,
                     "this readline version does not support history_data-size")
    @unittest.skipIf(is_editline,
                     "editline history_data size configuration is broken")
    def test_history_size(self):
        history_size = 10
        with temp_dir() as test_dir:
            inputrc = os.path.join(test_dir, "inputrc")
            with open(inputrc, "wb") as f:
                f.write(b"set history_data-size %d\n" % history_size)

            history_file = os.path.join(test_dir, "history_data")
